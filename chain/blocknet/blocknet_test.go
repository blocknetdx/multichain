...
var _ = Describe("Blocknet", func() {
	Context("when submitting transactions", func() {
		Context("when sending Blocknet to multiple addresses", func() {
			It("should work", func() {
				// Load private key, and assume that the associated address has
				// funds to spend. You can do this by setting blocknet_PK to the
				// value specified in the `./multichaindeploy/.env` file.
				pkEnv := os.Getenv("BLOCKNET_PK")
				if pkEnv == "" {
					panic("BLOCKNET_PK is undefined")
				}
				...
				// Submit the transaction to the blocknet node. Again, this
